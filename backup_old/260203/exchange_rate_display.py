...
汇率显示小组件 - 独立窗口，跟随桌宠移动
优化版：
1. 移除主题配置功能，使用固定的米白色背景
2. 在小组件上直接集成功能按钮（设置、Excel、提醒、关闭）
3. 跟随宠物移动，无需定时器
"""
...
class ExchangeRateDisplay(QWidget):
    ...
    # 信号
    settings_clicked = pyqtSignal()  # 设置按钮点击
    excel_clicked = pyqtSignal()     # Excel导出按钮点击
    alert_clicked = pyqtSignal()     # 提醒设置按钮点击
    ...
    def init_ui(self):
        ...
        self.settings_btn.setToolTip("汇率设置")
